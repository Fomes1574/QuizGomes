...
- Conclusão de desafio DIRECT/ASYNC (`ChallengeRoom`) separa a transição `COMPLETED` (CAS atômico em `sealHalf`) dos efeitos pós-conclusão. Os ledgers de XP, progressão e estatísticas têm `applied`: entradas pendentes são reaplicadas em batch atômico e só então marcadas como concluídas. Uma falha entre transição e efeito não perde XP, missão, streak ou estatística; o alarme do DO tenta novamente a partir de `challenge_answers`.
