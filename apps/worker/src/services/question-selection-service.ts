...
interface QuestionReader {
  pool(themeId: string): Promise<{ activeCount: number; id: string; version: number } | null>;
  secretBySlot(poolId: string, slot: number): Promise<SecretQuestionRecord | null>;
}

/**
 * Sorteio uniforme sobre os slots densos `1..N` do pool único do tema.
 *
...
export class QuestionSelectionService {
  ...
  async select(
    ...
    const pool = await this.questions.pool(themeId);
    ...
    try {
      ...
      if (error instanceof InsufficientQuestionPoolError) {
        throw new ApiError(409, error.code, 'Este tema ainda não possui perguntas ativas suficientes.');
      }
      throw error;
    }
